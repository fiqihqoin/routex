...
func (s *p2cAccountSelector) SelectAccount(ctx context.Context, vendorID string, accounts []domain.VendorAccount) (*domain.VendorAccount, error) {
	n := len(accounts)
	if n == 0 {
		return nil, fmt.Errorf("no accounts available for vendor %s", vendorID)
	}
	if n == 1 {
...
func (s *p2cAccountSelector) getAccountLoad(ctx context.Context, accountID string) float64 {
	inflightKey := fmt.Sprintf("stats:account:%s:inflight", accountID)
	latencyKey := fmt.Sprintf("stats:account:%s:latency", accountID)
	...
	// Weighted more towards in-flight for immediate load pressure
	return float64(inflight) + (latency / 100.0)
}

func (s *p2cAccountSelector) TrackInFlight(ctx context.Context, accountID string, delta int) error {
	key := fmt.Sprintf("stats:account:%s:inflight", accountID)
	return s.rdb.IncrBy(ctx, key, int64(delta)).Err()
}

func (s *p2cAccountSelector) TrackLatency(ctx context.Context, accountID string, latency time.Duration) error {
	key := fmt.Sprintf("stats:account:%s:latency", accountID)
